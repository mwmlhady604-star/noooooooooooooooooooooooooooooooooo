...
The Chat-Supervisor pattern includes a conversation closure feature for business registration inquiries:

1. **Information Confirmation**: When all necessary business registration information is collected, the agent summarizes the information in Iraqi Arabic for user confirmation.
2. **Delivery Preference**: Users can choose between receiving step-by-step instructions or a PDF summary.
3. **Email Delivery**: The system sends a confirmation email with the collected information in the user's preferred format.

For more details, see [CONVERSATION_CLOSURE.md](src/app/agentConfigs/chatSupervisor/CONVERSATION_CLOSURE.md)
